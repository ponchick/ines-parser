"""
Header lookup tables for iNES / NES 2.0

Includes mapper names, NES 2.0 submappers, Vs. System PPU/hardware types,
extended console types, and default expansion devices.

Mapper coverage follows MAME's ``nes_ines.hxx`` ``mmc_list`` (PCB identifiers
and comments from that file), merged with the curated names from the NESdev
Wiki summary table where those entries overlap. Many mapper numbers outside
the short wiki table only appear here via the MAME mapping.

NES 2.0 field names follow the NESdev Wiki specifications.

Sources:
  https://www.nesdev.org/wiki/List_of_mappers
  https://www.nesdev.org/wiki/Category:INES_Mappers
  https://www.nesdev.org/wiki/NES_2.0
  https://www.nesdev.org/wiki/NES_2.0_submappers
  https://github.com/mamedev/mame/blob/master/src/devices/bus/nes/nes_ines.hxx
"""
...
# Vs. PPU type (Header byte 13 low nibble when console type is Vs. System)
VS_PPU_TYPES: Dict[int, str] = {
    0x0: "RP2C03/RC2C03 (any)",
    0x1: "Reserved",
    0x2: "RP2C04-0001",
    0x3: "RP2C04-0002",
    0x4: "RP2C04-0003",
...
# Vs. Hardware type (Header byte 13 high nibble when console type is Vs. System)
VS_HW_TYPES: Dict[int, str] = {
    0x0: "Vs. Unisystem (normal)",
    0x1: "Vs. Unisystem (RBI Baseball protection)",
    0x2: "Vs. Unisystem (TKO Boxing protection)",
    0x3: "Vs. Unisystem (Super Xevious protection)",
    0x4: "Vs. Unisystem (Vs. Ice Climber Japan protection)",
    0x5: "Vs. Dual System (normal)",
    0x6: "Vs. Dual System (Raid on Bungeling Bay protection)",
}

# Extended console type (Header byte 13 low nibble when console type is Extended)
EXTENDED_CONSOLE_TYPES: Dict[int, str] = {
    0x0: "Regular NES/Famicom/Dendy (unused)",
    0x1: "Nintendo Vs. System (unused)",
    0x2: "PlayChoice-10 (unused)",
    0x3: "Famiclone with decimal mode",
    0x4: "NES/Famicom with EPSM",
    0x5: "V.R. Technology VT01 (STN palette)",
    0x6: "V.R. Technology VT02",
    0x7: "V.R. Technology VT03",
    0x8: "V.R. Technology VT09",
    0x9: "V.R. Technology VT32",
    0xA: "V.R. Technology VT369",
    0xB: "UMC UM6578",
    0xC: "Famicom Network System",
    0xD: "Reserved",
    0xE: "Reserved",
    0xF: "Reserved",
}

# Default expansion device (Header byte 15, bits 0-5)
EXPANSION_DEVICES: Dict[int, str] = {
    0x00: "Unspecified",
    0x01: "Standard NES/Famicom controllers",
    0x02: "NES Four Score/Satellite",
    0x03: "Famicom Four Players Adapter (simple)",
    ...
    0x24: "Dongda PEC Keyboard",
    0x25: "Bit Corp. Bit-79 Keyboard",
    0x26: "Subor Keyboard",
    0x27: "Subor Keyboard plus Macro Winners Mouse",
    0x28: "Subor Keyboard plus Subor Mouse via $4016",
    ...
    0x31: "Standard Controller (swapped axes/buttons)",
    0x32: "Excalibur Sudoku Pad",
    0x33: "ABL Pinball",
    0x34: "Golden Nugget Casino extra buttons",
    0x35: "Keda Keyboard",
    0x36: "Subor Keyboard plus Subor Mouse via $4017",
    ...
    0x3E: "Kingwon Keyboard",
    0x3F: "Zecheng Keyboard",
    0x40: "Subor Keyboard plus L90-rotated PS/2 mouse via $4017",
    0x41: "PS/2 Keyboard (UM6578) plus PS/2 Mouse via $4017",
    0x42: "PS/2 Mouse (UM6578)",
    0x43: "Yuxing Mouse via $4016",
    0x44: "Subor Keyboard plus Yuxing Mouse via $4016",
    0x45: "Gigggle TV Pump",
    0x46: "BBK Keyboard plus R90-rotated PS/2 mouse via $4017",
    0x47: "Magical Cooking",
    0x48: "SNES Mouse ($4017)",
    0x49: "Zapper ($4016)",
    0x4A: "Arkanoid Vaus Controller (Prototype)",
    0x4B: "TV Mahjong Game Controller",
    0x4C: "Mahjong Gekitou Densetsu Controller",
    0x4D: "Subor Keyboard plus X-inverted PS/2 mouse via $4017",
    0x4E: "IBM PC/XT Keyboard",
    0x4F: "Subor Keyboard plus Mega Book Mouse",
}

# (mapper, submapper) -> short name. Submapper 0 is often "Normal" / default iNES.
# Coverage follows NESdev "NES 2.0 submappers"; unknown pairs fall back in helpers.
SUBMAPPER_DATABASE: Dict[Tuple[int, int], str] = {
    # MMC1
    (1, 0): "Normal",
    ...
    # UxROM / CNROM / AxROM bus conflicts
    ...
    # Jaleco JF-16 / Cosmo Carrier / Holy Diver
    (78, 0): "Unspecified",
...
def _named_or_unknown(table: Dict[int, str], value: int) -> str:
...
def get_vs_ppu_type_name(value: Optional[int]) -> Optional[str]:
    if value is None:
        return None
    return _named_or_unknown(VS_PPU_TYPES, value)


def get_vs_hw_type_name(value: Optional[int]) -> Optional[str]:
    if value is None:
        return None
    return _named_or_unknown(VS_HW_TYPES, value)


def get_extended_console_type_name(value: Optional[int]) -> Optional[str]:
    if value is None:
        return None
    return _named_or_unknown(EXTENDED_CONSOLE_TYPES, value)


def get_expansion_device_name(value: Optional[int]) -> Optional[str]:
    if value is None:
        return None
    return _named_or_unknown(EXPANSION_DEVICES, value)


def get_submapper_name(mapper: int, submapper: Optional[int]) -> Optional[str]:
    """Return a short submapper name, or None if *submapper* is unset."""
    if submapper is None:
        return None
    name = SUBMAPPER_DATABASE.get((mapper, submapper))
    if name is not None:
        return name
    return f"Unknown ({submapper})"


def is_known_submapper(mapper: int, submapper: int) -> bool:
    return (mapper, submapper) in SUBMAPPER_DATABASE
