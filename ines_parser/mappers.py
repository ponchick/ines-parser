...
Coverage follows MAME's ``nes_ines.hxx`` ``mmc_list`` (PCB identifiers and
comments from that file), merged with the curated names from the NESdev Wiki
summary table where those entries overlap. Many mapper numbers outside the
short wiki table only appear here via the MAME mapping.

NESdev list (short summary): https://www.nesdev.org/wiki/List_of_mappers
Category index: https://www.nesdev.org/wiki/Category:INES_Mappers
MAME source of mmc_list: https://github.com/mamedev/mame/blob/master/src/devices/bus/nes/nes_ines.hxx
"""
